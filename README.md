...
**This reviews pull requests from branches of your own repository, and not from forks.** On a public repository GitHub gives a fork-triggered `pull_request` run a read-only token and no secrets, so on a fork the step has no engine to call and no permission to publish, and it goes green having done nothing. That is [a limitation with its own entry](#known-limitations), because a green run with no Gate check is easy to read as a pass.
