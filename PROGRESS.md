...
- [x] #65 - Data: scope completed-review uniqueness by repository -> done: 0003_scope_run_identity.sql replaces the global `(pr_number, head_sha)` constraint with repository-scoped `(repo_owner, repo_name, pr_number, head_sha)` uniqueness; preserves existing rows, permits shared PR/SHA values across repositories, and remains replay-safe.
