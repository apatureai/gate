 ...
 *
 * Tracking lives in `schema_migrations`; already-applied files are skipped, so
 * the runner is idempotent and safe to run automatically on every deploy.
...
export async function runMigrations(
  ...
  for (const file of files) {
    ...
    await exec.exec(sql);
    await exec.query("INSERT INTO schema_migrations (id) VALUES ($1)", [file]);
    applied.push(file);
  }
